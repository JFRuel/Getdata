...
5. From the data set in step 4, created a second, independent tidy data set with the average of each variable for each activity and each subject.
	...
	**Made the output tidy** by ensuring that 
	(1) Each variable measured is in one column
	(2) Each different observation of that variable is in a different row
	(3) There is one table for each "kind" of variable (in this case, there is only 1 output table)

	Also ensured that all variables have headings and that they are "human readable and free of invalid characters. 
	Ensured that variable are in different columns and that there are no are there no duplicate columns

================================================
## CODEBOOK - OUTPUT DATASET 
================================================

The "Mean_by_subject_and_activity.txt" dataset provides the mean of all the standard deviation and mean variables from the source dataset by subject, and by activity.

Here is the list of variables that can be found in the output (tidy) dataset, with format and a sample of data they contain: 

Descriptive variables:

 subject                             : int : Unique number identifying the 30 subects (from 1 to 30) 
 activity_label                      : Factor : with 6 levels - WALKING, WALKING_UPSTAIRS, WALKING_DOWNSTAIRS, SITTING
, STANDING, LAYING.
